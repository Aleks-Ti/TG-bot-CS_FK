...
async def choose_games_difficulty(message: Message):
    ...
    text = (
        "Прежде чем начать игру, выберите сложность.\nДля ознакомления, рекомендую сложность 3.\n\nЕсли хотите посложнее, "
        "оптимальный уровень 6 или 7.\nСложность 8 +, будет очень сложно, и может быть чертовски долго (V)O_o(V)"
    )
...
async def send_welcome(message: Message):
    ...
    await message.answer(
        ...
        "или может быть твоё имя в двоичном представлении?! Также как видит текст компьютер! Выбирай кнопку - **{}**\n\n"
        "Может хочешь конвертировать машинный код обратно в слова, "
        "то выбирай кнопку **{}**\n\n"
        "А может сыграем в игру **{}**?! Ищи кнопки внизу!\n\n"
        "Так же есть более сложная игра **{}**! Попробуешь испытать себя?!\n\n"
        "Выбирай игру вкусу, а лучше попробуй всё! Кнопки внизу 👇👇👇".format(
            user.first_name, mk.CONVERT_WORD_IN_BINARY_CODE, mk.CONVERT_BINARY_CODE_IN_WORD, mk.GAMES_GUESS_NUMBER, mk.HAORT_GAME,
        ),
        reply_markup=keyboard,
    )
